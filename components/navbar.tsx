...
export default function Navbar() {
  ...
  const handleSectionLink = (event: React.MouseEvent<HTMLAnchorElement>, sectionId?: string) => {
      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center space-x-8">
            ...
            >
              <Link href="/#contact" onClick={(event) => handleSectionLink(event, "contact")}>
          ...
          {/* Mobile Menu Button */}
          <Button aria-label={isOpen ? "Close navigation menu" : "Open navigation menu"} variant="ghost" size="icon" className="lg:hidden magnetic-btn" onClick={() => setIsOpen(!isOpen)}>
            {isOpen ? <X /> : <Menu />}
          </Button>
        </div>
              ...
              {navItems.map((item) => (
                <Link
                  ...
                  className={`text-left text-gray-300 hover:text-white transition-colors duration-300 py-2 magnetic-btn ${
                    item.section && activeSection === item.section ? "text-white border-l-2 border-blue-500 pl-3" : ""
                  }`}
                >
                  {item.name}
                </Link>
